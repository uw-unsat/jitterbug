...
(define (rv_offset insn off ctx)
  ...
  (define to (get-offset ctx (bvadd insn off)))
...
(define (emit_rv32_branch_r32 src1 src2 insn off ctx op)
  (define tmp1 (bpf2rv32 TMP_REG_1))
  (define tmp2 (bpf2rv32 TMP_REG_2))
...
(define (emit_rv32_branch_r64 src1 src2 insn off ctx op)
  (define tmp1 (bpf2rv32 TMP_REG_1))
  (define tmp2 (bpf2rv32 TMP_REG_2))
  ...
  (match code
    ...
    [(list (and class (or 'BPF_JMP 'BPF_JMP32)) code (and source (or 'BPF_X 'BPF_K)))
      (when (equal? source 'BPF_K)
        ...
        (set! src tmp2))
      (if is64
        (emit_rv32_branch_r64 dst src insn off ctx code)
        (emit_rv32_branch_r32 dst src insn off ctx code))]
